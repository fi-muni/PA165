...
public class CarShopStorageServiceImpl implements CarShopStorageService {
	...
	public void sellCar(Car car) throws RequestedCarNotFoundException {
	 	int carIndex = CarShopStorage.getInstancce().getCars().get(car.getType()).indexOf(car);
		if (carIndex == -1) {
			throw new RequestedCarNotFoundException("Car to sell was not found!");
		}
		CarShopStorage.getInstancce().getCars().get(car.getType()).remove(carIndex);
	}
